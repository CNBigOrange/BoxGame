...
#include "BoxManegerLevel2.h"
#include "NoteBook/NoteBookBlock.h"
#include "Kismet/GameplayStatics.h"

ABoxManegerLevel2::ABoxManegerLevel2()
{
	...
	TexArray.Emplace(ConstructorStatics3.FourTiao.Get());
...
void ABoxManegerLevel2::BeginPlay()
{
	Super::BeginPlay();
...
void ABoxManegerLevel2::SetBoxTexture(TArray<AActor*> Actors, TMap<int32, F_NumPair> Map)
{
	/************************获取场景中方块*************************/
	...
	for (auto Box : Actors)
	{
		if (Box) {
			...
			DynMaterial->SetTextureParameterValue("Texture", TexArray[i % 14]);

			TempBookBlock->BoxInfo.Type = BoxesMap.Find(i % 14)->Type;
			TempBookBlock->BoxInfo.TypeNum = BoxesMap.Find(i % 14)->TypeNum;
			TempBookBlock->BoxInfo.Texture = BoxesMap.Find(i % 14)->Texture;

			TempBookBlock->OwningManagerLevel2 = this;//设置Box的管理者
